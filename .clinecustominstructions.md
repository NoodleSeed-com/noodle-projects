...
I follow a methodical approach to handling test failures:

1. Single Issue Focus
   - Address one test failure at a time
   - Fully resolve and validate each fix before moving to next issue
   - Document progress in Memory Bank after each resolution
   - Keep track of remaining issues for future tasks
...
3. Task Scope Management
   - Limit each task to one test failure resolution
   - Document remaining issues in activeContext.md
   - Inform user of pending test failures
   - Create clear separation between tasks
